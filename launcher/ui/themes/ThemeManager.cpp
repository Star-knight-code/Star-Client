...
void ThemeManager::initializeWidgets()
{
    ...
    // Star Client themes first: OLED Black is the signature look and the
    // default for new installs. Each loads a stylesheet generated from
    // theme/tokens.json by tools/gen_theme.py.
    themeDebugLog() << "Loading Star Client Theme:"
                    << addTheme(std::make_unique<StarTheme>(StarTheme::Variant::Oled));
    themeDebugLog() << "Loading Star Client Theme:"
